...
\end{aligned}
$${#eq-twovariablfegen} 
where $f$ and $g$ are smooth functions of $x$ and $y$.
...
$(x^*,y^*)$ is defined to be a fixed point of @eq-twovariablfegen if 
...
Suppose that $(x^*,y^*)$ is a fixed point of equations @eq-twovariablfegen.
...
$$
A=\begin{pmatrix} 1 & -e^{-y} \\ 0 & -1 \end{pmatrix}.
$$
...
$$
Hence the FP at (-1,0) is a saddle.
...
y_x_nullcline=-np.log(-x)

fig,ax=plt.subplots()
ax.plot(x,y_x_nullcline,'--',[-3,3],[0,0],'--')
...
for i in range(len(x_vec)):
    for j in range(len(y_vec)):
        sol = q4(0,[X[j,i],Y[j,i]])
        U[j,i] = sol[0]
        V[j,i] = sol[1]

ax.streamplot(X,Y,U,V,color='grey',maxlength=10,integration_direction='both',density=1)
#ax.plot(x,y,'b',linewidth=2)
#ax.plot(x[0],y[0],'m*',markersize=10)


v_1 = np.array([1,0])
v_2 = np.array([1,2])

# normalize (direction only matters)
...
line_1 = np.outer(t,v_1)
line_2 = np.outer(t,v_2)

ax.plot(-1+line_1[:,0], line_1[:,1], 'r',
-1+line_2[:,0], line_2[:,1], 'r', linewidth=2)
...
(0,0) is a FP. The Jacobian matrix at (0,0) is
$$
A=\begin{pmatrix}
0& 1 \\
 -1 & 0
\end{pmatrix}. 
$$
Hence the FP of the linearised system is  a centre.
...
Hence the FP is a stable spiral for $a<0$. It is an unstable spiral for $a>0$.
...
The fixed points of @eq-competitionnondim are identified in the usual manner, i.e. by seeking $({n_1}^*,{n_2}^*)$  such that 
...
*  If $a_{12}>1$,  Population 2 also strongly competes with Population 1. In this case, there are four biologically relevant fixed points, two of which are stable (1,0) and (0,1). The coexistence fixed point is a saddle and thus unstable. 
...
At (1,0) and (-1,0) the FP of the linearised system is a centre.
...
fig,ax=plt.subplots()
ax.plot([-3,3],[0,0],'--')
ax.plot([-1,-1],[-3,3],'r--',[0,0],[-3,3],'r--',[1,1],[-3,3],'r--')
...
textstr=['$\dot{x}>0$','$\dot{x}<0$','$\dot{y}>0$','$\dot{y}<0$','$\dot{y}>0$']
xcoord=[0.4,0.4,-1.5,0,1.5]
ycoord=[0.4,-0.4,1.6,1.6,1.6]

for i in range(len(textstr)):
    ax.text(
        xcoord[i],ycoord[i], textstr[i],
        fontsize=10,
        verticalalignment='top',
        bbox=dict(boxstyle="round", facecolor="white", alpha=0.8)
    )
...
line_2[:,0], line_2[:,1], 'r', linewidth=2)
