...
Choose an appropriate value of the parameters $a$, $b$ and $c$ so that the solution captures the case of a particle of mass ($m$) equal to 3 subjected to a linear restoring force with spring constant ($k$) equal to 5.
...
$$
where $f$ is a prescribed function and $x_0$ is an initial condition. 

- phase space - put dependent variable on  Cartesian axes
- *phase point* - value of the solution at given time point 
- *vector field* - the derivative of the solution, i.e. $f$. 
