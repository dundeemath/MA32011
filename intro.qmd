...
Integrate each of @eq-linex and eq-nonlinex. Use the solutions to demonstrate that the principle of linear superpositions holds for @eq-lines but not for @eq-nonlinex.
...
The app in @fig-forwardEuler uses the Forward Euler method  to compute numerical solutions of the ODE
$$
\dot{x}=a+bx+cx^2
$$
...
$$
and the numerical solution agrees with the exact solution (dashed line).
